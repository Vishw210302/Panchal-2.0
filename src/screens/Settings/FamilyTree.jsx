...
const FamilyTree = ({ navigation }) => {
    ...
    const fetchFamilyData = async () => {
        try {
            ...
            Alert.alert('Error', 'Failed to load family tree');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleMemberPress = (member) => {
        navigation.navigate('MemberDetailsScreen', { userId: member._id });
    };

    const handleEditMember = (member, isRoot) => {
        if (isRoot) {
            Alert.alert('Info', 'You can edit your profile from the Profile section');
            return;
        }
        navigation.navigate('EditFamilyMember', { memberId: member._id });
    };
    ...
    const renderMemberCard = (member, isRoot = false, parentPerson = null, level = 0) => {
        ...
        const relationLabel = isRoot ? 'You' : getRelationshipLabel(member, parentPerson);
                        ...
                        <View style={styles.actionButtons}>
                            {!isRoot && (
                                <TouchableOpacity
                                    onPress={(e) => {
                                        e.stopPropagation();
                                        handleEditMember(member, isRoot);
                                    }}
    ...
    if (loading) {
        return (
            <View style={styles.container}>
                <HeaderBack navigation={navigation} title="Family Tree" />
        ...
        return (
            <View style={styles.container}>
                ...
                <View style={styles.emptyContainer}>
                    ...
                    <Text style={styles.emptyText}>Unable to load family tree</Text>
                </View>
            </View>
        );
    }
